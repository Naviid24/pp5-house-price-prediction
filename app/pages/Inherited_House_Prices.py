...
st.markdown(
    """
    These are the selected features that has the strong
...
st.markdown(
    """
    These are the estimated sale prices for four
    inherited houses based on their most effective
    features on SalePrice.
    """)
...
# ------------------------------
df_prices["Formatted Price"] = df_prices[
    "Predicted Price"].apply(lambda x: f"${x:,.2f}")
...
# ------------------------------
# Price Comparsion Chart Section
# ------------------------------
if st.checkbox("Show Price Comparison Chart"):
    ...
    chart = alt.Chart(df_prices).mark_bar().encode(
        ...
        y=alt.Y(
            'Predicted Price', title='Price ($)', scale=alt.Scale(zero=True)),
        tooltip=[alt.Tooltip('House'), alt.Tooltip(
