...
st.markdown(
    """
    In this section, we will evaluate how well our
    **Random Forest Model** predicts house prices.
    We will look at the **R² score** to understand how accurate our
    model is and visualize how close the predictions are to the actual prices.
    """)
...
st.markdown(
    """
    These plots show how the model’s predicted prices compare to
    the actual prices. If the dots are close to the red line,
...
st.markdown(
    """
    If the **Test R² score** is higher than **0.80**, it means the model is
    meeting the business goal of making accurate predictions.
    """)

if r2_test >= 0.80:
    st.success(
        """
        ✅ The model meets the business requirement with
        an R² ≥ 0.80 on the test set.
        """)
else:
    st.error(
        """
        ❌ The model does not meet the business
        requirement (R² < 0.80 on the test set).
        """)
